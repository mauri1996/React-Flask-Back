...
dfConsulta1 = df.select(df['Year'] , df['ArrDelay'] ,df['DepDelay'] , df['UniqueCarrier'], df['WheaterDelay'])
...
# Filtra todas aquellas Aerolinas que tengan retrasos
dfConsulta1 = dfConsulta1.filter(dfConsulta1['Retraso'] > 0)



# Filtra todas aquellas Aerolinas que tengan retrasos
dfConsulta1 = dfConsulta1.filter(dfConsulta1['WheaterDelay'] > 0)

#Cuenta cuantas Aerolinas tienen retrasos
dfConsulta1 = dfConsulta1.groupBy( 'Year' , 'UniqueCarrier' ).count()

#Guarda la informacion en un archivo llamado consulta3_.csv
dfConsulta1.repartition(1).write.format('com.databricks.spark.csv').save("./consulta3_.csv",header = 'true')
