...
dfConsulta1 = df.select(df['Year'] ,  df['Month'], df['ArrDelay'] ,df['DepDelay'] , df['UniqueCarrier'])
...
dfConsulta1 = dfConsulta1.groupBy( 'Year', 'Month', 'UniqueCarrier').count()

#Guarda la informacion en un archivo llamado consulta4_.csv
dfConsulta1.repartition(1).write.format('com.databricks.spark.csv').save("./consulta4_.csv",header = 'true')
